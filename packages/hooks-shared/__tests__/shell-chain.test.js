...
test("does not split inside single quotes", () => {
  assert.deepEqual(splitShellChain("echo 'foo && bar'"), ["echo 'foo && bar'"]);
});
...
test("handles nested $()", () => {
  assert.deepEqual(splitShellChain("echo $(foo $(bar && baz) qux) && ls"), [
...
test("splits git commit && rm .env — the motivating case", () => {
