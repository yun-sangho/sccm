---
description: File a bug report or feature request for the sccm-sandbox plugin at github.com/yun-sangho/sccm/issues. Auto-collects plugin version, OS, and recent context.
...
The user wants to file an issue against the **sccm-sandbox** plugin. Issues
...
2. **Auto-collect context. Do NOT ask the user for these.**
   ...
   - Recent context from this conversation: scan the last few user/assistant
     ...
     related to sccm-sandbox. Summarize in 2-4 lines. If nothing relevant
...
4. **Collect the missing essentials.** Ask the user only for what the body
   ...
   - **Title** (one line, will be prefixed with `[sccm-sandbox] `)
...
5. **Build the issue body** as Markdown. Mirror the field order of
   ...
   **Claude Code:** `<claude --version output, or "unknown">`
   ...
   ---
   _Filed via `/sccm-sandbox:report-issue` from inside Claude Code._
   ```
...
7. **On confirm, file the issue.** Try `gh` first:
   ...
   - Run:
     ...
     gh issue create \
       --repo yun-sangho/sccm \
       --title "[sccm-sandbox] <title>" \
       --body-file /tmp/sccm-issue-body.md \
       --label "plugin:sccm-sandbox" \
